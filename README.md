...
- Workflow
  ...
  - Shopify remote theme auto-reloading with [Shopify Theme Lab Reloader](https://github.com/uicrooks/shopify-theme-lab-plugins/tree/main/packages/reloader)
...
While `npm run start` task is running: The `shopify/` directory is being watched for changes and all changed files are uploaded to the Shopify remote server. After the upload is finished, a request is sent to a `localhost:port` address (specified in `package.json`) and the [Shopify Theme Lab Reloader](https://github.com/uicrooks/shopify-theme-lab-plugins/tree/main/packages/reloader) package reloads all connected Shopify store sites. *Open the web console to check if a site is connected.*
